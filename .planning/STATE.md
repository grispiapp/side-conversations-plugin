...
Last activity: 2026-09-22 — quick 260922-stt: compose Alıcı/Cc tek sol etiket kolonu + Cc progressive disclosure; test 595/595, tsc temiz. Canlı doğrulandı (standalone dev, 290px ve 1100px: her iki satırın input'u da `left:72`, etiketler `left:16`; tetikleyici Cc satırını açıp odağı `#compose-cc-input`'a taşıyor). Öncesinde 260918-fx7 CC (`ts.email_ccs`) desteği prod bundle'ıyla birlikte main'e push edildi (284a0bc) — **CC'nin canlı e-posta UAT'ı hâlâ yapılmadı**. (Phase 04.2 doğrulaması hâlâ `human_needed` — sıradaki iş değişmedi.)
